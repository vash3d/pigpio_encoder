# Rotary encoder class for ESP32 with micropython
# version: 0.2.5
...
class Rotary:
    ...
    def setup_pigpio_callbacks(self):
        clk_Pin = Pin(self.clk_gpio, Pin.IN, Pin.PULL_UP)
        self.clk_state = clk_Pin.value()
        dt_Pin = Pin(self.dt_gpio, Pin.IN, Pin.PULL_UP)
        self.dt_state = dt_Pin.value()
        sw_Pin = Pin(self.sw_gpio, Pin.IN, Pin.PULL_UP)
        self.sw_state = sw_Pin.value()
        
        clk_Pin.irq(handler=self.clk_gpio_call, trigger=Pin.IRQ_FALLING | Pin.IRQ_RISING)
        dt_Pin.irq(handler=self.dt_gpio_call, trigger=Pin.IRQ_FALLING | Pin.IRQ_RISING)
        
        if self.sw_gpio is not None:
            sw_Pin.irq(handler=self.sw_gpio_call, trigger=Pin.IRQ_FALLING | Pin.IRQ_RISING)
    ...
    def clk_gpio_call(self, pin):
        val = pin.value()
        if val < self.clk_state:
            self.clk_gpio_fall()
        elif val > self.clk_state:
            self.clk_gpio_rise()
        self.clk_state = val
...
#        print('dt_gpio_rise')
    ...
    def sw_gpio_call(self, pin):
        val = pin.value()
        if val < self.sw_state:
            self.sw_gpio_fall()
        elif val > self.sw_state:
            self.sw_gpio_rise()
        self.sw_state = val
